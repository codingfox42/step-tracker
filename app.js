...
if (localStorageAvailable) {
    ...
    try {
        const storedUsers = localStorage.getItem('stepTrackerUsers');
        if (storedUsers) {
            localUsers = JSON.parse(storedUsers);
        } else {
            localStorage.setItem('stepTrackerUsers', JSON.stringify(localUsers));
        }
        
        const storedGroups = localStorage.getItem('stepTrackerGroups');
        if (storedGroups) {
            localGroups = JSON.parse(storedGroups);
        } else {
            localStorage.setItem('stepTrackerGroups', JSON.stringify(localGroups));
        }
        
        const storedStepData = localStorage.getItem('stepTrackerStepData');
        if (storedStepData) {
            localStepData = JSON.parse(storedStepData);
        } else {
            localStepData = generateSampleData();
            localStorage.setItem('stepTrackerStepData', JSON.stringify(localStepData));
        }
...
try {
    ...
    console.log("Using local data storage instead");
    
    // Create mock Firebase references using local storage
    database = {
        ref: function(path) {
            return {
                ...
                set: function(newData) {
                    return new Promise((resolve) => {
                        if (path === 'users') {
                            localUsers = newData;
                            if (localStorageAvailable) {
                                localStorage.setItem('stepTrackerUsers', JSON.stringify(newData));
                            }
                        } else if (path === 'groups') {
                            localGroups = newData;
                            if (localStorageAvailable) {
                                localStorage.setItem('stepTrackerGroups', JSON.stringify(newData));
                            }
                        } else if (path === 'stepData') {
                            localStepData = newData;
                            if (localStorageAvailable) {
                                localStorage.setItem('stepTrackerStepData', JSON.stringify(newData));
                            }
                        }
                        resolve();
                    });
...
async function init() {
    ...
    try {
        ...
        // Check Firebase connection (but don't wait for it)
        checkFirebaseConnection().then(connected => {
            console.log("Firebase connection status:", connected);
            
            if (connected) {
                // Only try to initialize database if Firebase is connected
                    ...
                    if (stepDataRef) {
                        stepDataRef.on('value', snapshot => {
                            try {
                                ...
                                console.log("Step data updated:", stepData.length, "entries");
                                updateDashboard();
                            } catch (error) {
                ...
                console.log("Firebase not connected, using local functionality only");
            }
        }).catch(error => {
            console.error("Firebase connection check error:", error);
        });
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM Content Loaded in app.js - initializing app");
    
    // Add event listeners to critical elements
    addEventListeners();
...
function addEventListeners() {
    try {
        ...
        const submitStepsBtn = document.getElementById('submit-steps');
        if (submitStepsBtn) {
            submitStepsBtn.onclick = function() {
                console.log("Submit steps button clicked (direct onclick)");
                updateSteps();
            };
            console.log("Submit button onclick handler added");
        }
        ...
        tabButtons.forEach(button => {
            button.onclick = function() {
                console.log("Tab button clicked (direct onclick):", this.dataset.tab);
                switchTab(this.dataset.tab);
            };
            console.log("Tab button onclick handler added for", button.dataset.tab);
        });
        ...
        if (loginBtn) {
            loginBtn.onclick = function() {
                console.log("Login button clicked (direct onclick)");
                handleLogin();
            };
            console.log("Login button onclick handler added");
        }
        ...
        if (registerBtn) {
            registerBtn.onclick = function() {
                console.log("Register button clicked (direct onclick)");
                handleRegister();
            };
            console.log("Register button onclick handler added");
        }
        ...
        if (logoutBtn) {
            logoutBtn.onclick = function() {
                console.log("Logout button clicked (direct onclick)");
                handleLogout();
            };
            console.log("Logout button onclick handler added");
        }
        ...
        if (joinGroupBtn) {
            joinGroupBtn.onclick = function() {
                console.log("Join group button clicked (direct onclick)");
                handleJoinGroup();
            };
            console.log("Join group button onclick handler added");
        }
        ...
        if (createGroupBtn) {
            createGroupBtn.onclick = function() {
                console.log("Create group button clicked (direct onclick)");
                handleCreateGroup();
            };
            console.log("Create group button onclick handler added");
        }
